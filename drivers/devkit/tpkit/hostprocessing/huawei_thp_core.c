...
static void thp_prox_suspend_record_time(struct thp_core_data *cd)
{
	...
	THP_LOG_INFO("[Proximity_feature] TP early suspend at %lu secs %lu microseconds\n",
...
static bool thp_prox_timeout_check(struct thp_core_data *cd)
{
	...
	do_gettimeofday(&tv);
	THP_LOG_INFO("[Proximity_feature] check time at %lu seconds %lu microseconds\n",
	...
	delta_time /= 1000;
	THP_LOG_INFO("[Proximity_feature] delta_time = %d ms\n", delta_time);
